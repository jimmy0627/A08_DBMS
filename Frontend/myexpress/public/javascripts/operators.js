...
function applyFiltersAndSort() {
  ...
  // 2. 執行排序
  result.sort((a, b) => {
    // 輔助函數：從物件或 states 陣列中抓取數值
    const getVal = (obj) => {
        // 如果是星級，直接從主物件拿
        if (state.sortKey === 'rarity') return parseInt(obj.rarity) || 0;
        
        // 如果主物件有該欄位則優先使用
        if (obj[state.sortKey] !== undefined && obj[state.sortKey] !== null) {
            return parseFloat(obj[state.sortKey]) || 0;
        }

        // 否則從最高精英階段抓取
        if (obj.states && obj.states.length > 0) {
            const last = obj.states[obj.states.length - 1];
            return parseFloat(last[state.sortKey]) || 0;
        }
        return 0;
    };
...
function renderList(operators) {
  ...
  elements.listContainer.innerHTML = operators.map(op => {
    ...
    // 從 states 陣列中提取最高精英階段的數值 (後端回傳欄位為 states)
    const lastState = (op.states && op.states.length > 0) ? op.states[op.states.length - 1] : null;
    
    // 預算處理：如果 states 為空則顯示 '--'
    const hp = lastState ? lastState.hp : '--';
    const atk = lastState ? lastState.atk : '--';
    const def = lastState ? lastState.def : '--';
    const res = lastState ? lastState.res : '--';
    const cost = lastState ? lastState.cost : '--';
    const block = lastState ? lastState.block : '--';
    const redeploy = lastState ? `${lastState.redeploy}s` : '--';
    const atkSpd = lastState ? `${lastState.atk_spd}s` : '--';
    const avatarUrl = op.avatar_url || `/static/images/avatars/default.png`;

    return `
      <div class="operator-row rarity-${rarityNum}">
        <div class="col-portrait">
           <div class="portrait-box" style="background-image: url('${avatarUrl}'); background-size: cover; background-position: center;"></div>
        ...
        <div class="col-action">
          <a href="/operator.html?id=${id}" class="btn-details">
